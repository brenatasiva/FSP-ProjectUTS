...
$objMovie = new Movie($server, $user, $pwd, $db);
$objGambar = new Poster($server, $user, $pwd, $db);
$objPemain = new Pemain($server, $user, $pwd, $db);

$arr_movie_genre = array();
$res_genre = $objMovie->getGenreMovie();
		...
		while ($row = $res->fetch_assoc()) {
			...
			if (isset($arr_gambar_movie[$row['idmovie']])) {
								...
								foreach ($arr_kumpulan_gambar_pada_movie_ini as $idgambar) {
									echo "<img src='../w1/gambar/".$idgambar."'>";
								}
							}
